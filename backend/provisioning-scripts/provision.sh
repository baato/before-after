...
echo "[provision] rendering site for uuid=${4}"

# Split the bbox string into its four numbers for the map centre placeholders.
IFS=',' read -ra arr <<< "$2" || true

mkdir -p /appdata/provision/"${4}"
mkdir -p /appdata/beforestyles/"${4}"/
mkdir -p /appdata/afterstyles/"${4}"/

# Render the viewer template with the instance's parameters.
...
# Render the before/after map styles (tiles URL per epoch).
...
echo "[provision] ready: /provision/${4}"
